...
from pilott.delegation import TaskDelegator
from pilott.engine import LLMHandler
from pilott.knowledge import KnowledgeManager
...
class BaseAgent(ABC):
    ...
    async def _execute_with_llm(self, task: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """Execute task using LLM with dynamic prompt handling"""
        try:
            ...
            task_context = {
                ...
                "state": {
                    ...
                    "capabilities": {
                        "tools": context['tools'],
                        "knowledge_sources": context['knowledge']
                    }
                },
            ...
            # Get dynamic system prompt based on task type
            system_prompt = self.prompt_handler.get_system_prompt(
                f"{self.role}_{task.get('type', 'default')}",
                task_context
            )
            ...
            if hasattr(self, 'enhanced_memory') and self.enhanced_memory:
                memory_context = await self.enhanced_memory.semantic_search(str(task))
                if memory_context:
                    memories = "\n".join([f"- {item['text']}" for item in memory_context[:3]])
                    messages.append({
                        "role": "system",
            ...
            return {
                ...
                "role_alignment": "Executed according to defined role and goal"
            }
    ...
    def _format_task_content(self, task: Dict[str, Any]) -> str:
        """Format task content for LLM consumption"""
        # Remove sensitive or unnecessary fields
        safe_task = task.copy()
        safe_task.pop('credentials', None)
        safe_task.pop('api_keys', None)

        # Format based on task type
        task_type = task.get('type', 'default')
        if task_type == 'analyze':
            return f"Analyze the following content: {safe_task.get('content', '')}"
        elif task_type == 'extract':
            return f"Extract information from: {safe_task.get('content', '')}"
        else:
            return f"Process this task: {json.dumps(safe_task)}"
